...
TEST(PtsSetTest, EnvTest)
{
	...
	ASSERT_FALSE(env.lookup(ptr3).empty());
	EXPECT_TRUE(env.lookup(ptr3).has(obj3));

	EXPECT_TRUE(env.lookup(ptr4).empty());
...
TEST(PtsSetTest, StoreTest)
{
	...
	EXPECT_EQ(st1.size(), 1u);

	auto pSet1 = st1.lookup(obj1);
	EXPECT_FALSE(pSet1.empty());
	EXPECT_EQ(pSet1.size(), 2u);
	...
	EXPECT_EQ(st4.size(), 1u);

	auto pSet4 = st4.lookup(obj1);
	EXPECT_FALSE(pSet4.empty());
	EXPECT_EQ(pSet4.size(), 2u);
